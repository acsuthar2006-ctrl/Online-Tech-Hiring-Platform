...
2. **Media (Audio/Video)**: WebRTC sends audio/video over **UDP ports 40000-49999**.
   ...
   - **If you are on the same WiFi**: Your phone _can_ reach `192.168.x.x`, so it works.
...
3. **Connect with Ngrok (for HTTPS)**:
   ...
   - **Crucial**: Since your phone is on the same WiFi, the "Announced IP" (`10.140.100.77`) sent by the server _is_ reachable by your phone. The video should works.
...
- Ensure `./start_local.sh` detected the _correct_ IP (the WiFi one, not a Docker/VM one).

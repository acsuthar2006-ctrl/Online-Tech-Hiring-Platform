import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               *//* empty css                      */import"./auth-DKe50Cpr.js";import{a as s}from"./api-BDkOtMsK.js";import"./notifications-init-AG13z0We.js";import"./notifications-DCH5sCN5.js";const p=sessionStorage.getItem("companyId");let m=[],l=null;function g(){const e=s.getUserInfo();if(e){const o=document.getElementById("adminName");o&&(o.textContent=e.fullName||"Admin")}}function b(e){const o=document.getElementById("jobsContainer"),i=document.getElementById("emptyState");if(o){if(o.innerHTML="",!e||e.length===0){i&&(i.style.display="flex");return}i&&(i.style.display="none"),e.forEach(t=>{const a={OPEN:{bg:"#dcfce7",color:"#166534"},CLOSED:{bg:"#fee2e2",color:"#991b1b"},FILLED:{bg:"var(--blue-100)",color:"var(--blue-800)"}},n=a[t.status]||a.OPEN,c=(t.requiredExpertise?t.requiredExpertise.split(",").map(d=>d.trim()):[]).map(d=>`<span class="skill-tag">${d}</span>`).join(""),y=t.createdAt?new Date(t.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"";o.insertAdjacentHTML("beforeend",`
      <div class="job-card card">
        ...
        <div class="skill-tags" style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:12px;">${c}</div>
    ...
    `)})}}async function r(){if(!p){console.warn("No companyId in session. Cannot load positions.");const e=document.getElementById("emptyState");e&&(e.style.display="flex");return}try{m=await s.getCompanyPositions(p),b(m)}catch(e){console.error("Error loading positions:",e)}}window.openPostJobModal=function(){document.getElementById("postJobModal").style.display="flex",["positionTitle","jobDescription","salaryMin","salaryMax","requiredSkills","jobLocation"].forEach(e=>{const o=document.getElementById(e);o&&(o.value="")})};window.closePostJobModal=function(){document.getElementById("postJobModal").style.display="none"};window.postJob=async function(){const e=document.getElementById("positionTitle").value.trim(),o=document.getElementById("jobDescription").value.trim(),i=document.getElementById("salaryMin").value,t=document.getElementById("salaryMax").value,a=document.getElementById("requiredSkills").value.trim(),n=document.getElementById("jobLocation").value.trim();if(!e){alert("Please enter a position title.");return}const u=i&&t?`${i} - ${t}`:i?`From ${i}`:t?`Up to ${t}`:null;try{await s.createPosition({companyId:Number(p),positionTitle:e,jobDescription:o,salaryRange:u,requiredExpertise:a,location:n||null}),closePostJobModal(),await r()}catch(c){alert("Failed to create position: "+c.message)}};window.togglePositionStatus=async function(e,o){const i=o==="OPEN"?"CLOSED":"OPEN";try{await s.updatePositionStatus(e,i),await r()}catch(t){alert("Failed to update position status: "+t.message)}};window.confirmDeletePosition=function(e){confirm("Are you sure you want to delete this position? This action cannot be undone.")&&s.deleteCompanyPosition(e).then(()=>r()).catch(o=>alert("Delete failed: "+o.message))};window.openJobDetails=async function(e){l=e;const o=m.find(t=>t.id===e);if(!o)return;document.getElementById("jobTitle").textContent=o.positionTitle,document.getElementById("detailTitle").textContent=o.positionTitle,document.getElementById("detailStatus").textContent=o.status,document.getElementById("detailSalary").textContent=o.salaryRange||"Not specified",document.getElementById("detailSkills").textContent=o.requiredExpertise||"None specified",document.getElementById("detailDescription").textContent=o.jobDescription||"No description.";const i=document.getElementById("detailLocation");i&&(i.textContent=o.location||"Not specified"),document.getElementById("jobDetailsModal").style.display="flex",switchJobTab("overview")};window.closeJobDetailsModal=function(){document.getElementById("jobDetailsModal").style.display="none"};window.switchJobTab=async function(e){document.querySelectorAll(".tab-btn").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".tab-content").forEach(t=>t.classList.remove("active"));const o=[...document.querySelectorAll(".tab-btn")].find(t=>t.textContent.toLowerCase().includes(e));o&&o.classList.add("active");const i=document.getElementById(e+"Tab")||document.getElementById(e==="overview"?"overviewTab":e==="candidates"?"candidatesTab":"interviewersTab");if(i&&i.classList.add("active"),e==="candidates"&&l){const t=document.getElementById("candidatesList");if(t){t.innerHTML='<p class="text-muted">Loading...</p>';try{const a=await s.getPositionCandidates(l);a.length===0?t.innerHTML='<p class="text-muted">No candidates have applied yet.</p>':t.innerHTML=a.map(n=>`
          ...
          `).join("")}catch{t.innerHTML='<p class="text-muted">Failed to load interviewers.</p>'}}}};window.addEventListener("click",e=>{e.target.id==="postJobModal"&&closePostJobModal(),e.target.id==="jobDetailsModal"&&closeJobDetailsModal()});document.addEventListener("DOMContentLoaded",()=>{g(),r()});

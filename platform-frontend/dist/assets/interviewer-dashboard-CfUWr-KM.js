import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               *//* empty css                    */import"./interviewer-sidebar-BBk2cHuw.js";import{i as C}from"./notifications-CyUZvoHu.js";import{a as u}from"./api-BDkOtMsK.js";import{c as E,a as $}from"./dashboard-utils-BOnXSnva.js";let c=null,m=[],h=[],v={};async function b(){try{const t=document.getElementById("dashboard-content");if(t&&(t.innerHTML=E()),c=await u.getUserProfile(),c.role!=="INTERVIEWER"){window.location.href="/candidate/candidate-dashboard.html";return}m=await u.getUpcomingInterviewsForInterviewer(c.email),console.log("Scheduled interviews loaded:",m);try{const[e,n]=await Promise.all([u.getApprovedCompanies(c.id),u.getAllPositions()]);h=e,v={},(n||[]).forEach(i=>{v[i.id]=i.status}),console.log("Approved Companies loaded:",h)}catch(e){console.warn("Failed to load approved companies or positions:",e),h=[]}await I()}catch(t){console.error("Dashboard initialization error:",t);const e=document.getElementById("dashboard-content");e&&(e.innerHTML=$(t.message||"Failed to load dashboard"))}}async function I(){if(!document.getElementById("dashboard-styles")){const s=document.createElement("style");s.id="dashboard-styles",s.textContent=`
      .empty-state { text-align: center; padding: 30px; }
      .empty-icon { font-size: 48px; margin-bottom: 10px; }
      .empty-state h3 { color: #374151; margin-bottom: 5px; }
    `,document.head.appendChild(s)}const t=document.getElementById("dashboard-content");if(!t)return;const e=m.filter(s=>s.status==="SCHEDULED"||s.status==="IN_PROGRESS").length,i=m.filter(s=>s.status==="COMPLETED").length,a=new Set(m.map(s=>s.companyName).filter(Boolean));c.activeCompanies||a.size,c.totalEarnings&&`${c.totalEarnings}`;const r=await w(m,"upcoming"),d=await w(m,"completed"),l=`
    <!-- Header -->
  ...
  `;t.innerHTML=l,await C()}function S(t){return!t||t.length===0?'<p class="text-muted">You have not been approved for any companies yet.</p>':t.map(e=>{let n="";if(e.positions&&e.positions.length>0){const i=e.positions.filter(a=>v[a.positionId]==="OPEN");i.length>0?n=i.map(a=>`<button class="btn btn-outline btn-sm" style="margin-right: 8px; margin-top: 8px;" onclick="viewCandidates(${e.companyId}, ${a.positionId}, '${a.positionTitle.replace(/'/g,"\\'")}', '${e.companyName.replace(/'/g,"\\'")}')">
                  ${a.positionTitle} (View Candidates)
  ...
  `}).join("")}function D(t){return t.filter(e=>e.candidateOutcome&&e.candidateOutcome!=="PENDING"||e.interviewStatus==="SCHEDULED"||e.interviewStatus==="IN_PROGRESS"||e.interviewStatus==="COMPLETED"?!1:e.status==="APPLIED"||e.status==="SHORTLISTED"||e.status==="PENDING")}function k(t,e,n,i,a){return`schedule-an-interview.html?${new URLSearchParams({email:t.join(", "),positionTitle:e,companyName:n,...i?{companyId:i}:{},...a?{positionId:a}:{}}).toString()}`}window.viewCandidates=async(t,e,n,i)=>{const a=document.getElementById("candidatesModal"),r=document.getElementById("modalTitle"),d=document.getElementById("candidatesList");r.textContent=`Candidates for ${n}`,d.innerHTML="<p>Loading candidates...</p>",a.style.display="flex";try{const l=await u.getCandidatesForPositionAssigned(e,c==null?void 0:c.id);if(!l||l.length===0){d.innerHTML='<p class="text-muted">No candidates have applied for this position yet.</p>';return}const s=D(l),p=s.length>0?`<div style="margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid #e5e7eb;">
           <button
             class="btn btn-primary"
             style="width:100%;font-weight:600;"
             onclick="window.location.href='${k(s.map(o=>o.email),n,i,t,e)}'">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;">
            ...
            <div style="display:flex;gap:6px;">
              <button class="btn btn-sm" style="background:#16a34a;color:white;" onclick="markOutcomeInModal(event, ${o.interviewId}, 'ACCEPTED', ${e}, '${n.replace(/'/g,"\\'")}', '${i.replace(/'/g,"\\'")}')">✓ Accept</button>
              <button class="btn btn-sm" style="background:#dc2626;color:white;" onclick="markOutcomeInModal(event, ${o.interviewId}, 'REJECTED', ${e}, '${n.replace(/'/g,"\\'")}', '${i.replace(/'/g,"\\'")}')">✕ Reject</button>
            </div>
          </div>`:o.interviewStatus==="SCHEDULED"||o.interviewStatus==="IN_PROGRESS"?g=`<span class="badge badge-blue" style="font-size:12px;font-weight:500;padding:6px 10px;">${o.interviewStatus==="IN_PROGRESS"?"🔴 In Progress":"📅 Scheduled"}</span>`:o.status==="APPLIED"||o.status==="SHORTLISTED"||o.status==="PENDING"?g=`<button class="btn btn-primary btn-sm" onclick="openSchedulePage('${o.email}', '${o.fullName.replace(/'/g,"\\'")}', '${n.replace(/'/g,"\\'")}', '${i.replace(/'/g,"\\'")}'${t?`, ${t}`:""}, ${e})">Schedule Interview</button>`:g=`<span class="badge ${o.status==="REJECTED"?"badge-red":o.status==="OFFERED"||o.status==="ACCEPTED"?"badge-green":"badge-blue"}" style="font-size:12px;font-weight:500;">${o.status.replace("_"," ")}</span>`;return`
      ...
      `};d.innerHTML=p+l.map(y).join("")}catch(l){d.innerHTML=`<p style="color:red;">Failed to load candidates: ${l.message}</p>`}};window.closeCandidatesModal=()=>{document.getElementById("candidatesModal").style.display="none"};window.openSchedulePage=(t,e,n,i,a,r)=>{const d=new URLSearchParams({email:t,name:e,positionTitle:n,companyName:i,companyId:a,positionId:r});window.location.href=`schedule-an-interview.html?${d.toString()}`};window.closeScheduleModal=()=>{document.getElementById("scheduleModal").style.display="none"};window.handleScheduleSubmit=async t=>{t.preventDefault();const e=t.target.querySelector('button[type="submit"]');e.disabled=!0,e.textContent="Scheduling...";const i=`meet-${Math.random().toString(36).substring(2,10)}`,a={candidateEmail:document.getElementById("schedCandidateEmail").value,candidateName:document.getElementById("schedCandidateName").value,interviewerEmail:c.email,interviewerId:c.id,scheduledTime:document.getElementById("schedDate").value,title:document.getElementById("schedTitle").value,meetingLink:i,description:"Technical Interview generated via Dashboard",durationMinutes:parseInt(document.getElementById("schedDuration").value),interviewType:"TECHNICAL"};try{await u.scheduleInterview(a),alert("Interview scheduled successfully!"),window.closeScheduleModal(),b()}catch(r){alert("Failed to schedule: "+r.message),e.disabled=!1,e.textContent="Confirm Schedule"}};async function w(t,e="upcoming"){if(!t||t.length===0)return e==="upcoming"?'<p class="text-muted">No upcoming interviews.</p>':e==="completed"?'<p class="text-muted">No completed interviews.</p>':'<p class="text-muted">No interviews found.</p>';const n=e==="completed"?t.filter(a=>a.status==="COMPLETED"||a.status==="CANCELLED"):t.filter(a=>a.status==="SCHEDULED"||a.status==="IN_PROGRESS");return!n||n.length===0?e==="upcoming"?'<p class="text-muted">No upcoming interviews.</p>':e==="completed"?'<p class="text-muted">No completed interviews.</p>':'<p class="text-muted">No interviews found.</p>':[...n].sort((a,r)=>{const d=new Date((a.scheduledDate||"")+"T"+(a.scheduledTime||"00:00:00")),l=new Date((r.scheduledDate||"")+"T"+(r.scheduledTime||"00:00:00"));return e==="completed"?l-d:d-l}).slice(0,3).map(L).join("")}function L(t){const e=t.scheduledTime?t.scheduledTime.substring(0,5):"TBD",n=t.candidate?t.candidate.fullName:"Candidate",i=t.status==="COMPLETED"?"badge-green":"badge-blue";return`

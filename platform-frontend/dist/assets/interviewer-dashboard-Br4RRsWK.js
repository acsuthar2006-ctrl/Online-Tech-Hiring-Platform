import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               *//* empty css                    */import"./auth-DKe50Cpr.js";import{a as u}from"./api-BDkOtMsK.js";import{c as C,a as E}from"./dashboard-utils-BOnXSnva.js";import{i as $}from"./notifications-DCH5sCN5.js";let c=null,m=[],h=[],v={};async function b(){try{const t=document.getElementById("dashboard-content");if(t&&(t.innerHTML=C()),c=await u.getUserProfile(),c.role!=="INTERVIEWER"){window.location.href="/candidate/candidate-dashboard.html";return}m=await u.getUpcomingInterviewsForInterviewer(c.email),console.log("Scheduled interviews loaded:",m);try{const[e,n]=await Promise.all([u.getApprovedCompanies(c.id),u.getAllPositions()]);h=e,v={},(n||[]).forEach(a=>{v[a.id]=a.status}),console.log("Approved Companies loaded:",h)}catch(e){console.warn("Failed to load approved companies or positions:",e),h=[]}await I()}catch(t){console.error("Dashboard initialization error:",t);const e=document.getElementById("dashboard-content");e&&(e.innerHTML=E(t.message||"Failed to load dashboard"))}}async function I(){if(!document.getElementById("dashboard-styles")){const s=document.createElement("style");s.id="dashboard-styles",s.textContent=`
  ...
  `;t.innerHTML=l,await $()}function S(t){return!t||t.length===0?'<p class="text-muted">You have not been approved for any companies yet.</p>':t.map(e=>{let n="";if(e.positions&&e.positions.length>0){const a=e.positions.filter(i=>v[i.positionId]==="OPEN");a.length>0?n=a.map(i=>`<button class="btn btn-outline btn-sm" style="margin-right: 8px; margin-top: 8px;" onclick="viewCandidates(${e.companyId}, ${i.positionId}, '${i.positionTitle.replace(/'/g,"\\'")}', '${e.companyName.replace(/'/g,"\\'")}')">

import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{i as b}from"./notifications-CyUZvoHu.js";import"./dashboard-BJbQFGYd.js";import{a as u}from"./api-BDkOtMsK.js";import"./sidebar-ckr4qzLM.js";let m=[];function y(e){try{if(!e)return null;const[t,n]=e.split(":");if(t==null||n==null)return null;const o=new Date;return o.setHours(parseInt(t,10),parseInt(n,10),0,0),o.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}catch{return null}}function p(e){try{if(!e)return null;const t=new Date(e);return isNaN(t.getTime())?null:t.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}catch{return null}}function v(e,t){try{const n=new Date(e),o=new Date(t);if(isNaN(n.getTime())||isNaN(o.getTime()))return null;const c=Math.max(0,Math.round((o.getTime()-n.getTime())/6e4)),l=Math.floor(c/60),a=c%60;return l<=0?`${a}m`:a<=0?`${l}h`:`${l}h ${a}m`}catch{return null}}document.addEventListener("DOMContentLoaded",async()=>{const e=u.getUserInfo();if(!e){window.location.href="../../login/login.html";return}document.querySelectorAll("#userName, #profileName").forEach(n=>{n.textContent=e.fullName}),await E(),L(),await b()});async function E(){try{const e=u.getUserInfo();m=await u.getUpcomingInterviews(e.email),w("all")}catch(e){console.error("Failed to load schedule",e)}}function L(){const e=document.querySelectorAll(".filter-btn");e.forEach(t=>{t.addEventListener("click",()=>{e.forEach(o=>o.classList.remove("active")),t.classList.add("active");const n=t.getAttribute("data-filter");w(n)})})}function w(e){const t=document.getElementById("upcomingInterviewList"),n=document.getElementById("completedInterviewList"),o=document.getElementById("upcomingCount"),c=document.getElementById("completedCount"),l=m.filter(i=>["COMPLETED","CANCELLED"].includes(i.status)),a=m.filter(i=>["SCHEDULED","IN_PROGRESS"].includes(i.status));t&&(t.innerHTML=h(a)),n&&(n.innerHTML=h(l)),o&&(o.innerText=a.length),c&&(c.innerText=l.length);const s=document.getElementById("upcomingSection"),r=document.getElementById("completedSection");e==="all"?(s&&(s.style.display="block"),r&&(r.style.display="block")):e==="upcoming"?(s&&(s.style.display="block"),r&&(r.style.display="none")):e==="completed"&&(s&&(s.style.display="none"),r&&(r.style.display="block"))}function h(e){return!e||e.length===0?`
          ...
          <div class="timeline-item">
            ...
            <div class="interview-card">
              ...
              <div class="interview-actions">
                ...
                ${t.status==="COMPLETED"&&t.recordingUrl?`<button class="btn btn-primary btn-sm force-download-btn" style="margin-left: 5px;" data-url="${`/recordings/${t.recordingUrl}`}" data-filename="${t.recordingUrl}">Download Recording</button>`:""}
              </div>
            </div>
          </div>

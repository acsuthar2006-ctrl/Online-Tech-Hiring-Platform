...
async function initWebSocket(server) {
  ...
  wss.on("connection", (socket, req) => {
    ...
    socket.on("message", async msg => {
      ...
      if (data.type === 'produce') {
        ...
        if (transport) {
          const producer = await transport.produce({
            ...
            appData: { ...data.appData, channel: socket.channel, uid: socket.uid }
          });
      ...
      if (data.type === "leave") {
        if (socket.channel) {
          ...
          if (recorders.has(socket.channel)) {
            console.log(`[WebSocket] Stopping recorder for room ${socket.channel} due to user leave`);
            const recorder = recorders.get(socket.channel);
            recorder.stop();
      ...
      if (!socket.channel) {
        console.warn(`[WebSocket] User ${socket.uid} not in channel, ignoring ${data.type}`);
        return;
      }
    ...
    socket.on("close", (code, reason) => {
      console.log(`[WebSocket] ${socket.uid || 'Unknown'} disconnected: ${code} ${reason}`);
